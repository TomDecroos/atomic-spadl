...
def add_atomic_actiontypes(spadl_h5):
    with pd.HDFStore(spadl_h5) as spadlstore:
        print("...Inserting atomic actiontypes")
        actiontypesdf = pd.DataFrame(
            list(enumerate(atomic_actiontypes)), columns=["type_id", "type_name"]
        )
        spadlstore["atomic_actiontypes"] = actiontypesdf

def add_atomic_actions(spadl_h5):
    add_atomic_actiontypes(spadl_h5)
    with pd.HDFStore(spadl_h5) as spadlstore:
        ...
        for game_id in tqdm.tqdm(list(games.game_id), unit="game"):
            ...
            spadlstore[f"atomic_actions/game_{game_id}"] = actions

def extra_from_passes(actions):
    next_actions = actions.shift(-1)
    same_team = actions.team_id == next_actions.team_id
    
    passlike = ["pass", "cross", "throw_in", "freekick_short","freekick_crossed",
                "corner_crossed", "corner_short", "clearance", "goalkick"]
    pass_ids = list(actiontypes.index(ty) for ty in passlike)

    interceptionlike = ["interception","tackle","keeper_punch",
                        "keeper_save","keeper_claim","keeper_pick_up"]
    ...
    out = ((nex.type_id == actiontypes.index("goalkick")) & (~same_team)) | (nex.type_id == actiontypes.index("throw_in"))
    ar = atomic_actiontypes
    extra["type_id"] = -1
    extra["type_id"] = (extra.type_id
                        .mask(same_team,ar.index("receival"))
...
def extra_from_shots(actions):
    ...
    out = shot & (next_actions.type_id == actiontypes.index("goalkick")) & samegame & sameperiod
...
def extra_from_fouls(actions):
    ...
    extra["type_id"] = -1
    extra["type_id"] = (extra.type_id
                        .mask(yellow,ar.index("yellow_card"))
                        .mask(red,ar.index("red_card"))
                       )
    actions = pd.concat([actions, extra], ignore_index=True, sort=False)
    actions = actions.sort_values(["game_id", "period_id", "action_id"]).reset_index(
        drop=True
...
def convert_columns(actions):
    ...
    return actions[["game_id","period_id","action_id","time_seconds","timestamp",
